...
- build once, then test without building: `xcodebuild build-for-testing`, then `xcodebuild test-without-building`, against a booted simulator (`xcrun simctl bootstatus <udid> -b`).
- **an unsolved flake, and how to get past it.** the unit test host intermittently hangs before a single test runs: it logs "test daemon not ready" three times and the run fails as "the test runner hung before establishing connection", while `testmanagerd` is alive and logging. it has never once coincided with a real test failure — the UI tests in the same run pass. two explanations were tried and both were wrong: waiting for the simulator to finish booting, and building separately from testing. what has cleared it every time is running again, and on 16 september, after it hung twice in a row on a simulator that had been booted for hours, moving to a freshly booted device cleared it immediately. so: rerun, and if it repeats, shut that simulator down and boot another. do not read a hang as a failing build.
